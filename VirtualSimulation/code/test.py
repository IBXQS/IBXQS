...
class AGVDriveStraightTask:
    def __init__(self):
        ...
        self.carry = True
        self.count=0
        self.loop_times = []             # { changed code }
        self.start_time = time.time()    # { changed code }
        self.path_points = [
    ...
    def _move_agv(self):
        ...
        if self.current_point_index < len(self.path_points):
            ...
            loop_time = time.time() - self.start_time      # { changed code }
            self.loop_times.append(loop_time)              # { changed code }
            print(f"Loop {self.count} took {loop_time:.2f} seconds.")  # { changed code }
            self.start_time = time.time()                  # { changed code }
