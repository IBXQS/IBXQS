...
class MyGridLayout(BoxLayout):
   def __init__(self, **kwargs):
       ...
       self.add_widget(ColoredLabel(
           text='Capacity of table (default: 1)',
           font_size=20,
           color=(1, 1, 1, 1),
           bg_color=(0.8, 0.3, 0.3, 1)
       ))
   ...
   def on_submit(self, instance):
       if self.submit.text == "Submit":
           ...
           self.capacity = int(self.capacity_input.text or 1)
           self.label.text = f'AGV speed: {self.agv_speed}, AGV turn: {self.agv_round}, Ingredient: {self.ingredient}, Capacity: {self.capacity}'
   ...
   def get_input_values(self):
       if self.root:
           return {
               ...
               'capacity': getattr(self.root, 'capacity',None)
           }
       return None
...
glb_capacity=input_values['capacity']
...
class AGVDriveStraightTask:
    def __init__(self):
        ...
        self.path_points = [
            ...
            {"position": np.array([5.4, 9.5, -0.32]), "rotation": 90, "stoptime": 30},   # turn - down
            {"position": np.array([3.35, 9.5, -0.32]), "rotation": 90, "stoptime": 30}   # end - down
        ]
    ...
    def run(self):
        ...
        while simulation_app.is_running() and not self.stop_sim:
            ...
            if self.world.is_playing():
                ...
                if self.my_agv.paused==True and self.my_agv.carry==True and self.my_agv.current_point_index==3:     #agv at table
                    self.arm1_pick=True
                ...
                if self.my_agv.paused==True and self.my_agv.carry==True and self.my_agv.current_point_index==2:  #if agv just arrive at the machine
                    self.arm2_pick=True
                    self.my_agv.carry=False
                ...
                if self.my_agv.paused and self.my_agv.current_point_index == 4 and self.my_agv.carry:
                    self.my_agv.carry = False
                    if (self.ontable>=glb_capacity):
                        self.product += glb_capacity
                        self.ontable -=glb_capacity
